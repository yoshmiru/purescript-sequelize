...
foldWhere' :: forall a. Where a -> Map.Map String Foreign
foldWhere' w = case w of
  And cs -> foldAnd cs
  Or cs -> foldOr cs
  Is key val -> foldIs key val

foldAnd :: forall a. Array (Where a) -> Map.Map String Foreign
foldAnd [] = Map.empty
foldAnd xs = Map.singleton "$and" (unsafeToForeign $ map foldWhere' xs)

foldOr :: forall a. Array (Where a) -> Map.Map String Foreign
foldOr [] = Map.empty
foldOr xs = Map.singleton "$or" (unsafeToForeign $ map foldWhere' xs)

foldIs :: String -> Term -> Map.Map String Foreign
